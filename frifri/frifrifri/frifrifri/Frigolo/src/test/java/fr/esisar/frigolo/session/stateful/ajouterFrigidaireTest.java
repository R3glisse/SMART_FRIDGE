...
public class ajouterFrigidaireTest {

    private static final Logger log = Logger.getLogger(ajouterFrigidaireTest.class.getName());
    ...
    public static Archive createTestArchive() {
        return ShrinkWrap.create(WebArchive.class, "testajouterFrigidaireCont.war")
    ...
    public void testgetAllUnites() throws Exception {
        ...
        Admin.setFrigidaireNom("Test");

        Admin.ajouterFrigidaire();

        List<FrigidaireEJBEntity> TestFrig = fridgeEJB.findFrigidaires();
        Assert.assertFalse(TestFrig.isEmpty());
        log.info("\n\n\nTest ajouterFrigidaire() is functional\n\n\n");

        Long IdFridge = TestFrig.get(0).getIdFrigidaire();
        Admin.setFrigidaireId(IdFridge);
        Admin.suppressionFrigidaire();

        TestFrig = fridgeEJB.findFrigidaires();
        Assert.assertTrue(TestFrig.isEmpty());
        log.info("\n\n\nTest suppressionFrigidaire() is functional\n\n\n");
