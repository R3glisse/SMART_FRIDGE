...
public class mesuresNumeriquesFromCapteurIdTest {

    private static final Logger log = Logger.getLogger(mesuresNumeriquesFromCapteurIdTest.class.getName());
    ...
    @Inject
    private MesureNumeriqueEJB MesNumEJB;
    @EJB
    ...
    public static Archive createTestArchive() {
        return ShrinkWrap.create(WebArchive.class, "testgetmesuresNumeriquesFromCapteurIdCont.war")
    ...
    @Test
    public void testmesuresNumeriquesFromCapteurId() throws Exception {
        ...
        List<UniteEJBEntity> TestUni = unitEJB.findUnites();
        ...
        Assert.assertFalse(TestUni.isEmpty());
        Assert.assertFalse(TestCapNum.isEmpty());

        Long IdCaptNumTmp = TestCapNum.get(0).getIdCapteur();
        Long IdUniteTmp = TestUni.get(0).getIdUnite();
        MesNumEJB.ajouterMesureNumerique(new Float("3.0"), IdCaptNumTmp, IdUniteTmp);

        Admin.setCapteurNumeriqueId(TestCapNum.get(0).getIdCapteur());

        Admin.mesuresNumeriquesFromCapteurId();
        List<MesureNumeriqueEJBEntity> TestMesNum = Admin.getMesuresNumeriques();

        Assert.assertFalse(TestMesNum.isEmpty());

        log.info("\n\n\nTest getmesuresNumeriquesFromCapteurId() is Validated\n\n\n");

        Admin.mesuresNumeriquesFromCapteurIdRAZ();

        TestMesNum = Admin.getMesuresNumeriques();

        Assert.assertTrue(TestMesNum.isEmpty());

        log.info("\n\n\nTest getmesuresNumeriquesFromCapteurIdRAZ() is Validated\n\n\n");
